...
- The plist enables `--strict-coreml`, sets `--max-seconds 70` so a single AUTO
  request can cover up to ~256 non-whitespace characters in one CoreML bucket,
  passes `--chars-per-second 5.5` (calibrated from measured Japanese explanatory
  speech ~5.37 chars/s — the previous `4.0` overestimated generation seconds and
